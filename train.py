...
# ----------------------------------------------------------------------------------
# MODIFICATIONS:
# - Added gradient accumulation with optional DDP `no_sync` to reduce memory and communication.
#   * New param: `accum_steps` (CLI: `--accum-steps`, default=1). Loss is divided by `accum_steps`.
#   * Optimizer/scheduler step only at the end of each accumulation window.
# - Switched `model.zero_grad()` to `optimizer.zero_grad(set_to_none=True)` at accumulation window start.
# - Replaced `torch.no_grad()` with `torch.inference_mode()` in evaluation/diagnostics.
# - Added optional gradient clipping (`--max-grad-norm`, default=0 disables).
# - Added optional torch.compile before DDP (`--torch-compile`, `--compile-mode`).
# - Added optional enablement of Flash/Mem-Efficient SDPA (`--enable-sdp-flash`) if model uses SDPA.
# - Exposed DataLoader knobs via `params` (`pin_memory`, `num_workers`, `prefetch_factor`, `persistent_workers`).
# - Minor: `GradScaler(enabled=...)` only when FP16 is used (BF16 uses no scaler).
# - NEW (speed): Optional DDP static graph + bucket size; fewer hot-loop empty_cache; throttle logging via --log-every-n-steps.
# - NEW (speed): Default AMP dtype = BF16 if supported; fall back to FP16 (with GradScaler) otherwise.
# - NEW (speed): Reuse latitude tensor instead of rebuilding every iteration.
# ----------------------------------------------------------------------------------
...
python train.py \
  --yaml_config v2.0/config/PANGU_S2S.yaml --config S2S \
  --accum-steps 8 --max-grad-norm 1.0 \
  --torch-compile --compile-mode reduce-overhead \
  --enable-sdp-flash --ddp-static-graph \
  --log-every-n-steps 20 --amp-dtype auto
...
# >>> NEW imports for memory/perf tweaks
import contextlib
try:
    # Deprecated in newer PyTorch; we won't rely on it for global toggles.
...
def compute_weighted_acc(da_fc, da_true, clim=None, weighted=True, mean_dims=xr.ALL_DIMS, **kwargs):
    ...
    if clim is not None:
        if True:
            ...
            # Reorder and transpose to match forecast data
...
class Trainer():
    def __init__(self, params, world_rank):
        ...
        # ---- AMP dtype selection (bf16 preferred if supported) ----
        ...
        if amp_choice == "bf16" and torch.cuda.is_available():
            ...
            if torch.cuda.is_available() and torch.cuda.is_bf16_supported():
                ...
                self.amp_dtype = torch.float32  # CPU fallback (autocast disabled)
        ...
        # Setup model/opt/scheduler/loss
        # self.setup_model()  # called later explicitly to keep your flow
        logging.info('Params %s', params)  # fixed formatting

    def setup_model(self):
        # Set up model
        self.mask_bool, self.land_mask = self.get_land_mask_bool()
        self.model = self.get_model()
        # GradScaler only when FP16 (BF16 doesn't need it)
    ...
    def check_land_ocean_variables(self) -> None:
        """Update booleans about land/ocean variables."""
        self.has_land = False
        ...
        if hasattr(self.params, 'mask_output'):
            # keep structure; uses self.params
            self.mask_output = self.params.mask_output

    def create_dirs(self, run_uuid:int)->tuple[str, str, str]:
        """Create output directories to store spectra_output, image, and ACC plots."""
    ...
    def get_model(self):
        """Get the model based on nettype."""
        if self.params.nettype == 'pangu_plasim':
        ...
        if getattr(params, "torch_compile", False):
            try:
    ...
    def setup_scheduler(self):
        if self.params.scheduler == 'ReduceLROnPlateau':
            ...
            #         self.optimizer, max_lr=self.params.lr, total_steps=total_steps,
            ...
            #     )
                ...
                # self.scheduler = torch.optim.lr_scheduler.OneCycleLR(
                #     self.optimizer, max_lr=params.lr, total_steps=total_steps,
                #     steps_per_epoch=steps_per_epoch, last_epoch=(self.startEpoch-1)*steps_per_epoch,
                #     pct_start=pct_start, div_factor=div_factor, final_div_factor=final_div_factor
                # )
                self.scheduler = torch.optim.lr_scheduler.OneCycleLR(
                self.optimizer, max_lr=self.params.lr,
    ...
    def setup_loss_fun(self):
        """Set up loss function to return the loss for pl, sfc and diagnostic."""
        self.loss_obj_pl = 0
        self.loss_obj_sfc = 0
    ...
    def train(self):
        ...
        for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            torch.cuda.empty_cache()  # coarse-grained only
            ...
            for i, data in enumerate(train_data_loader):
                ...
                if self.params.mode == "test" and i >= self.params.test_iterations:
                    ...
                    with torch.inference_mode():
                        ...
                        latitudes = self.lat_t  # reused tensor
                        ...
                        diagnostic_logs = self.diagnostic_log_per_iter(
                            ...
                            train_batch_loss=loss, train_batch_loss_sfc=loss_sfc,
                            train_batch_loss_upper_air=loss_pl, train_batch_loss_diagnostic=loss_diagnostic,
                            train_batch_loss_vae=loss_vae, train_mean_norm_lwrmse=mean_norm_lwrmse
                        )
                    ...
                    # NOTE: Removed per-iteration empty_cache() for performance
    ...
    def cal_loss(self, input_surface, constant_boundary_data, varying_boundary_data, input_upper_air,
        ...
        """Forward + loss for surface, upper air, and optional diagnostic outputs."""
        ...
        with autocast(device_type="cuda",
            ...
            if self.params.has_diagnostic:
                output_surface, output_upper_air, output_diagnostic, mu, sigma, mu2, sigma2 = self.model(
                    input_surface, constant_boundary_data, varying_boundary_data, input_upper_air,
    ...
    def inti_valid_loss(self, lead_times_steps) -> tuple:
        """Initialise validation loss variables."""
        if self.params.has_diagnostic:
        ...
        if self.params.has_diagnostic:
            ...
            multi_step_rmse = {f"valid_lwrmse_sfc_{step}step": torch.zeros(1, dtype=torch.float32, device=self.device) for step in lead_times_steps} |\
                {f"valid_lwrmse_pl_{step}step": torch.zeros(1, dtype=torch.float32, device=self.device) for step in lead_times_steps}
        return valid_loss_diag if self.params.has_diagnostic else None, valid_buff, valid_loss, valid_loss_sfc, valid_loss_pl, valid_steps, valid_surface_lwrmse, valid_upper_air_lwrmse, (valid_diagnostic_lwrmse if self.params.has_diagnostic else None), multi_step_losses, multi_step_rmse
    ...
    def validate_one_epoch(self):
        ...
        with torch.inference_mode():
            for i, data in tqdm(enumerate(self.valid_data_loader, 0), total=nb, bar_format='{l_bar}{bar:30}{r_bar}{bar:-10b}'):
                ...
                # Tensors for ACC calc (all time steps)
                ...
                # NOTE: Removed per-batch empty_cache() for performance
                valid_steps += 1.
    ...
    def restore_checkpoint(self, checkpoint_path):
        ...
        try:
            ...
            for key, val in checkpoint['model_state'].items():
                name = key[7:]; new_state_dict[name] = val
...
if __name__ == '__main__':
    ...
    parser.add_argument("--torch-compile", action="store_true", help="Enable torch.compile on the model before DDP")
    parser.add_argument("--compile-mode", default="reduce-overhead", type=str, help="torch.compile mode")
    parser.add_argument("--enable-sdp-flash", action="store_true", help="Enable Flash/mem-efficient SDPA kernels if available")
    # >>> NEW speed toggles
    parser.add_argument("--ddp-static-graph", action="store_true", help="Enable DDP static graph optimization")
    parser.add_argument("--ddp-bucket-cap-mb", default=25, type=int, help="DDP gradient bucket size in MB")
    parser.add_argument("--amp-dtype", default="auto", choices=["auto", "bf16", "fp16"], help="AMP compute dtype")
    ...
    try:
        ...
        params['torch_compile'] = bool(getattr(args, 'torch_compile', False))
        ...
        params['ddp_bucket_cap_mb'] = int(getattr(args, 'ddp_bucket_cap_mb', 25))
        params['amp_dtype'] = getattr(args, 'amp_dtype', 'auto')
        params['log_every_n_steps'] = int(getattr(args, 'log_every_n_steps', 20))
    except Exception as _e:
        logging.warning(f"Failed to wire CLI flags to params: {_e}")

    # DataLoader knobs (effective only if your get_data_loader reads them)
    ...
    try:
        if not hasattr(params, 'num_workers'):
            params['num_workers'] = min(8, os.cpu_count() or 2)
    except Exception:
        if not hasattr(params, 'num_workers'):
            params['num_workers'] = 2

    # Enable SDPA Flash/Mem-efficient kernels using official flags (works across PyTorch 2.x)
    if getattr(params, 'enable_sdp_flash', False) and torch.cuda.is_available():
        try:
            torch.backends.cuda.enable_flash_sdp(True)
            torch.backends.cuda.enable_mem_efficient_sdp(True)
            torch.backends.cuda.enable_math_sdp(False)
            logging.info("Enabled Flash and memory-efficient SDPA kernels")
        except Exception as _e:
            logging.info(f"SDPA kernel enable skipped: {_e}")
