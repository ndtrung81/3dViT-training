# ----------------------------------------------------------------------------------
# faster_train.py —  S2S trainer optimized for H100 and B300 (Blackwell) GPUs
#
# Changes from H100-only version:
#   - Auto-detects GPU architecture (Hopper vs Blackwell) and sets defaults accordingly
#   - NCCL/CUDA env vars scale to the GPU generation
#   - torch.compile handles SM_103 (Blackwell) Triton issues gracefully
#   - Adds --amp-dtype fp8 option via NVIDIA Transformer Engine (Blackwell only)
#   - All training logic is IDENTICAL — same model, same loss, same data loading
#
# Search for "# B300:" to see every change from the original.
# ----------------------------------------------------------------------------------
...
# ------------------------------------
# B300: GPU architecture detection — must happen before setting env vars
# ------------------------------------
def _detect_gpu_arch():
    """Detect GPU architecture. Returns 'blackwell', 'hopper', or 'unknown'."""
    if not torch.cuda.is_available():
        return 'unknown'
    try:
        major, minor = torch.cuda.get_device_capability(0)
        name = torch.cuda.get_device_name(0).lower()
        if major >= 10:  # SM_100+ = Blackwell
            return 'blackwell'
        elif major == 9:  # SM_90 = Hopper
            return 'hopper'
        else:
            return 'unknown'
    except Exception:
...
# ------------------------------------
# B300: Architecture-aware environment tuning
# NCCL and CUDA settings scale to the GPU generation.
# ------------------------------------
if GPU_ARCH == 'blackwell':
    # Blackwell: NVLink 5, higher bandwidth, larger buffers
    os.environ.setdefault("NCCL_BUFFSIZE", "16777216")     # 16MB
    ...
    # Hopper defaults (matches the H100 sbatch script)
    os.environ.setdefault("NCCL_BUFFSIZE", "16777216")     # 16MB (same as sbatch)
    os.environ.setdefault("NCCL_NTHREADS", "512")
    os.environ.setdefault("NCCL_SOCKET_NTHREADS", "8")
    os.environ.setdefault("NCCL_NSOCKS_PERTHREAD", "4")
    os.environ.setdefault("NCCL_MAX_NCHANNELS", "32")

# Common NCCL settings (same for both architectures, matches sbatch)
...
# Common CUDA settings (matches sbatch)
...
# ------------------------------------
# B300: Architecture-aware defaults
# ------------------------------------
DEFAULT_LOSS_WEIGHTS = {'surface': 0.25, 'diagnostic': 0.25, 'upper_air': 1.0}

if GPU_ARCH == 'blackwell':
    GPU_DEFAULTS = {
        'ddp_bucket_cap_mb': 512,   # Larger for NVLink 5 bandwidth
        'prefetch_factor': 4,
        'num_workers': 8,
    ...
    GPU_DEFAULTS = {
        'ddp_bucket_cap_mb': 256,   # Original H100 value
        'prefetch_factor': 4,
        'num_workers': 8,
        'log_every_n_steps': 50,
        'metrics_every': 200,
...
# ------------------------------------
# B300: Transformer Engine import (optional, for FP8)
# ------------------------------------
_TE_AVAILABLE = False
try:
...
# B300: Log detected architecture
logging.info(f"Detected GPU architecture: {GPU_ARCH}")
if _TE_AVAILABLE:
    logging.info("Transformer Engine available (FP8 supported)")
...
class Trainer():
    def __init__(self, params, world_rank):
        ...
        if getattr(params, 'enable_sdp_flash', False):
            logging.warning("--enable-sdp-flash disabled - incompatible with Pangu 3D attention")
        ...
        self.log_every = max(1, int(getattr(params, "log_every_n_steps", GPU_DEFAULTS['log_every_n_steps'])))
        self.metrics_every = int(getattr(params, "metrics_every", GPU_DEFAULTS['metrics_every']))
        self.grad_stats_every = int(getattr(params, "grad_stats_every", GPU_DEFAULTS['grad_stats_every']))
        
        # B300: Track whether we're using FP8
        self.use_fp8 = False
        ...
        # B300: AMP dtype handling — added fp8 and fp32 options
        amp_choice = getattr(self.params, "amp_dtype", "bf16")
        if amp_choice == "fp32":
            self.amp_dtype = torch.float32
            logging.info("Using FP32 (AMP disabled)")
        elif amp_choice == "fp8":
            if GPU_ARCH != 'blackwell' and GPU_ARCH != 'hopper':
                logging.warning("FP8 requested but GPU is not Hopper/Blackwell. Falling back to BF16.")
                amp_choice = "bf16"
            elif not _TE_AVAILABLE:
                logging.warning("FP8 requested but Transformer Engine not installed. Falling back to BF16.")
                logging.warning("Install with: pip install transformer_engine[pytorch]")
                amp_choice = "bf16"
            else:
                self.use_fp8 = True
                self.amp_dtype = torch.bfloat16  # FP8 uses BF16 as the master dtype
                logging.info(f"FP8 training enabled via Transformer Engine (GPU: {GPU_ARCH})")

        if not self.use_fp8:
            if amp_choice == "bf16" and torch.cuda.is_available() and torch.cuda.is_bf16_supported():
                self.amp_dtype = torch.bfloat16
                logging.info("Using BF16 mixed precision")
            elif amp_choice == "fp16" and torch.cuda.is_available():
                self.amp_dtype = torch.float16
                logging.info("Using FP16 mixed precision")
            else:
                self.amp_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16

        # B300: FP8 recipe setup
        # NOTE: te.fp8_autocast only accelerates Transformer Engine layers (te.Linear, etc).
        # PanguModel_Plasim currently uses standard nn.Linear/nn.Conv layers, so FP8
        # won't give the full speedup until those are swapped to TE equivalents.
        # This run still serves as a correctness test and baseline for future TE integration.
        if self.use_fp8:
            ...
            logging.info(
                "NOTE: FP8 autocast is enabled but PanguModel_Plasim uses nn.Linear, not te.Linear. "
                "Full FP8 speedup requires swapping model layers to Transformer Engine equivalents."
            )
    ...
    def setup_model(self):
        ...
        self.scaler = GradScaler(enabled=(torch.cuda.is_available() and self.amp_dtype == torch.float16 and not self.use_fp8))
    ...
    def _compile_model(self, model):
        ...
        try:
            ...
            # B300: More helpful error message for Blackwell Triton issues
            logging.warning(f"torch.compile failed: {e}")
            if GPU_ARCH == 'blackwell' and 'sm_103' in str(e).lower() or 'ptxas' in str(e).lower():
                logging.warning(
                    "This looks like a Triton SM_103 compatibility issue. Options:\n"
                    "  1. Use NGC container (nvcr.io/nvidia/pytorch:25.04-py3)\n"
    ...
    def _wrap_ddp(self, model):
        ...
        # B300: Use architecture-aware default bucket size
        bucket_cap = int(getattr(self.params, "ddp_bucket_cap_mb", GPU_DEFAULTS['ddp_bucket_cap_mb']))
    ...
    def get_dataset(self):
        ...
        dl_kwargs = {
            ...
            'prefetch_factor': GPU_DEFAULTS['prefetch_factor'],
            'num_workers': int(os.environ.get("SLURM_CPUS_PER_TASK", GPU_DEFAULTS['num_workers'])),
        }
    ...
    def train_one_epoch(self):
        ...
        for year_idx, train_data_loader in enumerate(self.train_data_loaders):
            ...
            for i, data in enumerate(data_iter):
                ...
                # B300: Forward + loss with optional FP8 autocast
    ...
    def cal_loss(self, inp_sfc, const_bd, vary_bd, inp_ua, tgt_diag, tgt_sfc, tgt_ua):
        ...
        # B300: FP8 autocast wraps around the normal AMP autocast
        fp8_ctx = (te.fp8_autocast(enabled=True, fp8_recipe=self.fp8_recipe)
                   if self.use_fp8 else contextlib.nullcontext())
...
if __name__ == '__main__':
    ...
    # Optimization flags (defaults work for both H100 and B300)
    ...
    parser.add_argument("--ddp-bucket-cap-mb", default=None, type=int,
                        help="DDP bucket size in MB. Default: auto (256 for Hopper, 512 for Blackwell)")
    parser.add_argument("--ddp-powersgd", action="store_true")
    parser.add_argument("--ddp-fp16-compress", action="store_true", default=True)
    # B300: Added fp8 and fp32 as valid choices
    parser.add_argument("--amp-dtype", default="bf16", choices=["bf16", "fp16", "fp8", "fp32"])
    ...
    # B300: Batch size override (for memory saturation sweeps)
    parser.add_argument("--batch-size-override", default=None, type=int,
                        help="Override batch size from YAML config. Used for GPU memory saturation sweeps.")
    # B300: Data directory override (for local vs GPFS comparison)
    parser.add_argument("--data-dir-override", default=None, type=str,
                        help="Override data_dir from YAML config. Use to compare local NVMe vs GPFS.")
    # B300: Disable TF32 for pure FP32 baseline
    ...
    for k, v in vars(args).items():
        if k not in ['yaml_config', 'config', 'local_rank']:
            # B300: If ddp_bucket_cap_mb not explicitly set, use architecture default
            if k == 'ddp_bucket_cap_mb' and v is None:
                params[k] = GPU_DEFAULTS['ddp_bucket_cap_mb']
            else:
    ...
    # B300: Disable TF32 for pure FP32 precision baseline
    if args.no_tf32:
    ...
    if world_rank == 0:
        ...
        params.log()
        # B300: Log architecture and GPU info at startup
        logging.info(f"GPU Architecture: {GPU_ARCH}")
        logging.info(f"GPU Defaults: {GPU_DEFAULTS}")
        logging.info(f"Using {world_size} of {torch.cuda.device_count()} available GPUs")
